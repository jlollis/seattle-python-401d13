    ...
    def validate_roll(self, roll):
        ...
        while True:
            ...
            if self.validate(roll, keepers):
                return self._strip_superflous(keepers)
            else:
                self._print('No way pal')
                self._print(roll)

    # 111456
    def _strip_superflous(self, roll):
        score = self.calculate_score(roll)
        goodies = []
        for i, val in enumerate(roll):
            temp = list(roll[:])
            temp.pop(i)
            temp_score = self.calculate_score(temp)
            if score > temp_score:
                goodies.append(val)

        return tuple(goodies)
...
if __name__ == "__main__":
    game = Game()
    # game.play()
    keepers = game._strip_superflous([1,1,1,5,2])
    print(keepers)
